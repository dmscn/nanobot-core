...
class TelegramChannel(BaseChannel):
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if inline_buttons:
            ...
            logger.debug(
                "Sending message with inline keyboard: {} callback_id={}",
                inline_buttons,
                callback_id,
            )
    ...
    def _build_inline_keyboard(
        self, buttons: list[dict], callback_id: str
    ) -> InlineKeyboardMarkup | None:
        """Build InlineKeyboardMarkup from button definitions.

        Button format:
        [
            {"id": "btn_id", "label": "Button Text", "instruction": "action for agent", "metadata": {...}},
    ...
    def _store_callback(self, callback_id: str, buttons: list[dict], message_id: int) -> None:
        ...
        if callback_id not in self._pending_callbacks:
            self._pending_callbacks[callback_id] = {"buttons": {}, "message_ids": []}
